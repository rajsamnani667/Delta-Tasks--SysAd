...
app.config['MYSQL_DATABASE_PASSWORD'] = 'earth143'
...
@app.route('/moms', methods = ['POST','GET'])
def moms():
    if(session.get('logged_in')!=True):
        return redirect(url_for('signin'))
    if request.method=='GET':
        ...
        cursor.close()
        return Render('moms.html',table=table)
...
if __name__ == "__main__":
    app.run(debug=True,host='0.0.0.0', port=5000) # remove parameters inside run() for local use